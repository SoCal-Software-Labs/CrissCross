...
impl StoreMut<Key> for MemoryStorage {
    async fn insert_schema(self, schema: &Schema) -> MutResult<Self, ()> {
        ...
        let bin = match bincode::serialize(&item) {
            Ok(b) => b,
        ...
        put_rows(
            self.pid,
            "__meta__".as_bytes(),
            vec![(table_name.as_bytes().to_vec(), bin)],
        )
        .await
        .try_self(self)
    }
    ...
    async fn delete_schema(self, table_name: &str) -> MutResult<Self, ()> {
        delete_rows(
            self.pid,
            "__meta__".as_bytes(),
            vec![table_name.as_bytes().to_vec()],
        )
        .await
        .try_self(self)
    }
    ...
    async fn insert_data(self, table_name: &str, rows: Vec<Row>) -> MutResult<Self, ()> {
        let to_insert = rows
            .iter()
            .map(|row| {
                let bin = match bincode::serialize(row) {
                    Ok(b) => b,
        ...
        put_rows(self.pid, table_name.as_bytes(), to_insert)
            .await
            .try_self(self)
    }
    ...
    async fn delete_data(self, table_name: &str, keys: Vec<Key>) -> MutResult<Self, ()> {
        let to_delete = keys.iter().map(|key| key.id.clone()).collect();

        delete_rows(self.pid, "__meta__".as_bytes(), to_delete)
            .await
            .try_self(self)
    }
}
                ...
                Some(caller) => {
                    ...
                    msg_env.send_and_clear(&(caller.pid), |env| {
                        ...
                        match str::from_utf8(&caller.payload) {
                            Err(e) => {
                                (atoms::error(), "invalid utf8", hydrated_reference).encode(env)
                            }
                            Ok(v) => match glue.execute(v) {
                                Ok(statement_result) => {
                                    // let mut bin =
                                    ...
                                    match serde_json::to_value(statement_result) {
                                        Ok(result) => match to_term(env, result) {
                                            Ok(term) => {
                                                (atoms::ok(), term, hydrated_reference).encode(env)
                                            }
                                            Err(e) => {
                                ...
                                Err(e) => {
                                    (atoms::error(), e.to_string(), hydrated_reference).encode(env)
                                }
                            },
                        }
                    });
...
fn receive_result<'a>(
    ...
    let results = if success {
        if has_key {
            (
                true,
                Some((key.as_slice().to_vec(), result.as_slice().to_vec())),
            )
        } else {
            (true, None)
        }
...
async fn put_rows(
    ...
    let callback_token =
        rustler::resource::ResourceArc::new(CallbackTokenResource { token: token });

    let mut msg_env = rustler::OwnedEnv::new();
    msg_env.send_and_clear(&pid, |env| {
        (atoms::put(), table, values, callback_token).encode(env)
    });
    ...
    match option_res {
        Ok(res) => {
            ...
            match success {
                true => Ok(()),
                false => Err(Error::StorageMsg(
                    format!("Foreign PUT to Elixir Failed: {:?}", r).into(),
                )),
            }
        }
        Err(_) => Err(Error::StorageMsg(
            format!("Foreign PUT To Elixir Failed").into(),
        )),
    }
}
...
async fn delete_rows(pid: rustler::types::Pid, table: &[u8], values: Vec<Vec<u8>>) -> Result<()> {
    ...
    let mut msg_env = rustler::OwnedEnv::new();
    msg_env.send_and_clear(&pid, |env| {
        (atoms::delete(), table, values, callback_token).encode(env)
    });
    ...
    match option_res {
        Ok(res) => {
            ...
            match success {
                true => Ok(()),
                false => Err(Error::StorageMsg(
                    format!("Foreign DELETE to Elixir Failed: {:?}", r).into(),
                )),
            }
        }
        Err(_) => Err(Error::StorageMsg(
            format!("Foreign DELETE To Elixir Failed").into(),
        )),
    }
}
...
async fn get_row(
    ...
    let mut msg_env = rustler::OwnedEnv::new();
    msg_env.send_and_clear(&pid, |env| {
        (atoms::get(), table, value, callback_token).encode(env)
    });
    ...
    match option_res {
        Ok(res) => {
            ...
            match success {
                ...
                false => Err(Error::StorageMsg(
                    format!("Foreign GET to Elixir Failed: {:?}", r).into(),
                )),
            }
        }
        Err(_) => Err(Error::StorageMsg(
            format!("Foreign GET To Elixir Failed").into(),
        )),
    }
}
...
async fn start_scan(pid: rustler::types::Pid, table: &[u8]) -> Result<rustler::types::Pid> {
    ...
    let callback_token =
        rustler::resource::ResourceArc::new(ScanCallbackTokenResource { token: token });

    let mut msg_env = rustler::OwnedEnv::new();
    msg_env.send_and_clear(&pid, |env| {
        (atoms::start_scan(), table, callback_token).encode(env)
    });
...
async fn scan_next(table_name: String, pid: rustler::types::Pid) -> Result<Option<(Key, Row)>> {
    ...
    let mut msg_env = rustler::OwnedEnv::new();
    msg_env.send_and_clear(&pid, |env| (atoms::scan_next(), callback_token).encode(env));
    ...
    match option_res {
        Ok(res) => {
            ...
            match success {
                true => match res {
                    Some((key, r)) => match bincode::deserialize(&r[..]) {
                        Ok(row) => Ok(Some((
                            Key {
                        ...
                        Err(e) => Err(Error::StorageMsg(format!("Failed to decode row: {:?}", e))),
                    },
                    None => Ok(None),
                },
                false => Err(Error::StorageMsg(
                    format!("Foreign GET to Elixir Failed: {:?}", res).into(),
                )),
            }
        }
        Err(_) => Err(Error::StorageMsg(
